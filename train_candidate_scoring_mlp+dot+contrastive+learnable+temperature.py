...
# =========================================================
# 4. mlp-dot + contrastive 모델
#    Label-aware candidate span scoring
#    score = MLP score + dot-product score
#    auxiliary loss = learnable-temperature-scaled CE(dot_scores, gold_label)
# =========================================================
...
class RECandidateScoringModel(nn.Module):
    def __init__(
        ...
        # Learn log-temperature so that actual temperature is always positive.
...
def compute_auprc_metrics(all_labels, all_probs, id2label, no_relation_id=0):
    """
    Multi-class relation classification에서 one-vs-rest 방식으로 AUPRC 계산.
    - macro_auprc: 모든 label 포함
    - macro_auprc_without_no_relation: no_relation 제외
    - micro_auprc: 전체 label을 펼쳐서 micro 평균
    """
    ...
    # one-hot gold label matrix: [num_examples, num_labels]
    ...
    def safe_macro_auprc(label_ids):
        ...
        for label_id in label_ids:
            # 해당 label의 positive sample이 없으면 AUPRC 계산에서 제외
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(progress, start=1):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                outputs = model(**batch)
                scores = outputs["scores"]
                dot_scores = outputs["dot_scores"]

                relation_loss = criterion(scores, labels)

                temperature = model.get_temperature()
                contrastive_loss = criterion(
                    dot_scores / temperature,
                    labels
                )

                loss = (
                    args.relation_alpha * relation_loss
                    + args.contrastive_alpha * contrastive_loss
                )
            ...
            progress.set_postfix({
                ...
                "rel": float(relation_loss.detach().cpu()),
                "ctr": float(contrastive_loss.detach().cpu()),
            ...
            torch.save(
                {
                    ...
                    "scoring_type": "mlp_dot_contrastive_learnable_temperature",
                },
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/re_klue_roberta_base_mlp_dot_contrastive_learnable_temp")
    parser.add_argument("--eval_model_dir", type=str, default="outputs/re_klue_roberta_base_mlp_dot_contrastive_learnable_temp/best_model")
